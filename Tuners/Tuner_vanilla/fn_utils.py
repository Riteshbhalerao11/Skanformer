...
# def save_data_tools(tokenizer, src_vocab, tgt_vocab, src_itos, tgt_itos, tools_dir):
#     os.makedirs(tools_dir, exist_ok=True)

#     with open(os.path.join(tools_dir, 'tokenizer.pkl'), 'wb') as f:
#         pickle.dump(tokenizer, f)

#     torch.save(src_vocab, os.path.join(tools_dir, 'src_vocab.pth'))
#     torch.save(tgt_vocab, os.path.join(tools_dir, 'tgt_vocab.pth'))

#     with open(os.path.join(tools_dir, 'src_itos.json'), 'w') as f:
#         json.dump(src_itos, f)
#     with open(os.path.join(tools_dir, 'tgt_itos.json'), 'w') as f:
#         json.dump(tgt_itos, f)

#     print(f"All components saved in '{tools_dir}'.")

# def load_data_tools(tools_dir):

#     tokenizer_file = os.path.join(tools_dir, 'tokenizer.pkl')
#     with open(tokenizer_file, 'rb') as f:
#         tokenizer = pickle.load(f)

#     src_vocab_file = os.path.join(tools_dir, 'src_vocab.pth')
#     tgt_vocab_file = os.path.join(tools_dir, 'tgt_vocab.pth')
#     src_vocab = torch.load(src_vocab_file)
#     tgt_vocab = torch.load(tgt_vocab_file)

#     src_itos_file = os.path.join(tools_dir, 'src_itos.json')
#     tgt_itos_file = os.path.join(tools_dir, 'tgt_itos.json')
#     with open(src_itos_file, 'r') as f:
#         src_itos = json.load(f)
#     with open(tgt_itos_file, 'r') as f:
...
def init_distributed_mode(config):
    ...
    # print(f"DEVICE ID FOR PROCESS {pid} : {rank}")
    # dist.init_process_group(backend=config.backend,
    #                         rank=rank, world_size=config.world_size)
