...
class KANFeedForwardBlock(nn.Module):
    ...
    def __init__(self, in_size: int, ff_dims: Union[List[int],int], grid_size: int = 8, device: Union[str, int] = 'cpu') -> None:
        super().__init__()
        self.ffn = torch.nn.ModuleList()

        self.ffn.append(SineKANLayer(
                in_size, ff_dims, grid_size=grid_size, device=device, is_first=True))
        self.ffn.append(SineKANLayer(
                ff_dims, in_size, grid_size=grid_size, device=device, is_first=False))
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, features: int, self_attention_block: MultiHeadAttentionBlock, cross_attention_block: MultiHeadAttentionBlock, 
                 feed_forward_block : Union[KANFeedForwardBlock, FeedForwardBlock]  ,dropout: float) -> None:
        super().__init__()
        self.self_attention_block = self_attention_block
...
def build_kanformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int=512, 
    ...
    for i in range(N):
        ...
        if i == N-1:
            decoder_block = DecoderBlock(d_model, decoder_self_attention_block, decoder_cross_attention_block, kan_block, dropout)
        else:
